...
int main() {
    ...
    // debug(ans)
    // for (int i = 0; i < pcnt; ++i) {
    //     for (int64 h = (L + pp[i] - 1) / pp[i] * pp[i]; h <= R; h += pp[i]) {
    //         int64 j = h - L;
    //         while (val[j] % pp[i] == 0) val[j] /= pp[i];
    //         int x = pp[i];
    //         if (is[pp[i]] == 0) {
    //             ans = (ans + mu[x] * ksm((R / x - (L - 1) / x), n) % MOD) % MOD;
    //         }
    //         is[pp[i]] = 1;
    //     }
    // }
    ...
    //     if (val[i] > 1) {
    //         int x = val[i];
    //         ans = (ans + -1 * ksm((R / x - (L - 1) / x), n) % MOD) % MOD;
    //     }
    // }
    // for (int i = 0, x; i < pcnt; ++i) {
    //     x = pp[i];
    //     if (x > R) break;
    //     ans = (ans + mu[x] * ksm((R / x - (L - 1) / x), n) % MOD) % MOD;
    // }
